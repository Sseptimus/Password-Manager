...
login_wallpapers = [r"Password-Generator\Assets\login_wallpapers\cave_edited.png",
                    r"Password-Generator\Assets\login_wallpapers\tower_edited.png"]
headerBackground = PhotoImage(
    file=r"Password-Generator\Assets\Element backgrounds\header_background.png")
tabBackground = PhotoImage(
    file=r"Password-Generator\Assets\Element backgrounds\tab_background.png")
...
def setup_manager(): #Sets up the main password manager window and places all the elements on screen
    ...
    geometry = "700x500+" + \
        str(int(screen_width/2 - 350)) + "+" + str(int(screen_height/2 - 250))
    manager.geometry(geometry)
    set_tabs()
    header = Label(manager, image=headerBackground, borderwidth=0)
    header.place(x=0, y=0)
    add_password = Button(manager, image=new_password_Background,
                          width=50, height=50, borderwidth=0, command=new_password)
    add_password.place(y=37, x=640)
    next_button = Button(manager, text="→", font=(
        "Arial", 20), background="#0e0118", foreground="#ffffff", command=next_page)
    next_button.place(y=440, x=650)
    previous_button = Button(manager, text="←", font=(
        "Arial", 20), background="#0e0118", foreground="#ffffff", command=previous_page)
...
def set_tabs(): #places all the main elements where they need to be on the pages, either on or off screen
    ...
    for i in passwords:
        ...
        if passwords.index(i) < 3:
            ...
            account_labels.append(Label(manager, text=accounts[passwords.index(
                i)], font=("Arial", 20), fg="#ffffff", background="#0e0118"))
            account_labels[passwords.index(i)].place(
                x=20, y=155+passwords.index(i)*100)
            password_labels.append(Label(manager, text=len(
                i)*("*"), font=("Arial", 20), fg="#ffffff", background="#0e0118"))
            view_buttons.append(Button(manager, command=partial(popup, passwords.index(
                i)), text="View Password", font=("Arial", 15), background="#0e0118", foreground="#ffffff"))
            view_buttons[passwords.index(i)].place(
                x=540, y=155+passwords.index(i)*100)
            if len(i) > 6:
            ...
            tabs.append(Label(manager, text="Text", width=700,
                        height=100, image=tabBackground, borderwidth=0))
            tabs[passwords.index(i)].place(
                x=700*(total_pages-1), y=125+(passwords.index(i)-(total_pages*3))*100)
            account_labels.append(Label(manager, text=accounts[passwords.index(
                i)], font=("Arial", 20), fg="#ffffff", background="#0e0118"))
            account_labels[passwords.index(i)].place(
                x=700*(total_pages-1)+20, y=155+(passwords.index(i)-(total_pages*3))*100)
            password_labels.append(Label(manager, text=len(
                i)*("*"), font=("Arial", 20), fg="#ffffff", background="#0e0118"))
            view_buttons.append(Button(manager, command=partial(popup, passwords.index(
                i)), text="View Password", font=("Arial", 15), background="#0e0118", foreground="#ffffff"))
            view_buttons[passwords.index(i)].place(
                x=540, y=155+(passwords.index(i)-(total_pages*3))*100)
            if len(i) > 6:
                password_labels[-1].config(text="******")
                password_labels[-1].place(x=490-36+700*(total_pages-1),
                                          y=155+(passwords.index(i)-(total_pages*3))*100)
            else:
                password_labels[-1].place(x=490-len(i)*5+700*(total_pages-1),
                                          y=155+(passwords.index(i)-(total_pages*3))*100)
...
def new_password_entry(): #Saves new password and updates all lists with new info
    ...
    tabs.append(Label(manager, width=700, height=100,
                image=tabBackground, borderwidth=0))
    account_labels.append(Label(manager, text=account_input.get(), font=(
        "Arial", 20), fg="#ffffff", background="#0e0118"))
    if len(password_input.get()) > 6:
        password_labels.append(Label(
            manager, text="******", font=("Arial", 20), fg="#ffffff", background="#0e0118"))
    else:
...
def login_setup(): #Sets up the login window, placing all elements on screen
    ...
    geometry = "500x500+" + \
        str(int(screen_width/2 - 250)) + "+" + str(int(screen_height/2 - 250))
    ...
    enter_button = Button(login_page, text="Enter", font=(
        "Arial", 15), background="#0e0118", foreground="#ffffff", command=login_check)
